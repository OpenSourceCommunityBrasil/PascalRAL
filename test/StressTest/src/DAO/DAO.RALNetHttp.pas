...
begin
  ...
  try
    case aMethod of
      rtmGET:
        begin
          StatusCode := FClientAPI.Get(BaseURL);
          Result := StatusCode = 200;
        end;
      rtmDELETE:
        begin
          StatusCode := FClientAPI.Delete(BaseURL);
          Result := StatusCode = 200;
        end;
      rtmPOST:
        begin
          StatusCode := FClientAPI.POST(BaseURL);
          Result := StatusCode = 201;
        end;
      rtmPUT:
        begin
          StatusCode := FClientAPI.PUT(BaseURL);
          Result := StatusCode = 201;
        end;
      rtmPATCH:
        begin
          StatusCode := FClientAPI.PATCH(BaseURL);
          Result := StatusCode = 201;
        end;
    end;
    if not Result then
...
begin
  ...
  try
    ...
    Result := Result and (FClientAPI.ResponseText.Contains(AExpectedResponse));
