...
begin
  ...
  Move(FCRC32, Result[0], SizeOf(LongWord));

  inherited;
end;

function TRALCRC32.GetBuffer(AIndex: IntegerRAL): Pointer;
begin
  Result := Pointer(@FBuffer[AIndex]);
end;

function TRALCRC32.GetBufLength: IntegerRAL;
begin
  Result := 64;
end;
...
begin
  ...
  for vInt := 0 to AIndex - 1 do
    UpdateCRC32(FBuffer[vInt]);
end;

procedure TRALCRC32.Initialize;
begin
  inherited;
  FCRC32 := $FFFFFFFF;
  FillChar(FBuffer, Sizeof(FBuffer), 0);
...
procedure TRALCRC32.UpdateCRC32(AOctet: Byte);
begin
  FCRC32 := Crc_32_Tab[Byte(FCRC32 xor LongInt(AOctet))] xor
            ((FCRC32 shr 8) and $00FFFFFF);
end;
