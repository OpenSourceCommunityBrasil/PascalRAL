 ...
 * OnValidateServerCert, then what the platform concluded) keeps deciding here
 * too, instead of a second rule growing inside this bridge.
 *
 * It is called DURING the TLS handshake, which is the whole point: answering
 * "no" aborts the connection before a single byte of the request - the token
 * included - has been written.
 */
public interface RalCertJudge {
   ...
   * @param platformTrusted whether Android's own trust store accepted the
   *                        chain - what RAL calls TRALCertInfo.Trusted
