/// .tar.gz (.tgz) extraction, LCL-free: the mORMot2 static files come this way.
/// Understands ustar (name + prefix), GNU tar long names (type 'L') and pax
/// headers (type 'x', with path=). Symbolic links are only created outside
/// Windows; other types (device, fifo) are skipped. Entries with '..' or an
/// absolute path are refused: the file comes from the network.
unit RALInst.Tar;
...
/// ARetirarNiveis drops the first folders of every path (1 = the top folder)
...
begin
  ...
  if vGz = nil then
  begin
    AErro := Format(emTarAbrir, [AArquivo]);
    Exit;
  end;
  try
    vNomeLongo := '';
    ...
    repeat
          ...
          begin
            ...
            try
              if not LerTudo(vGz, vTamanho, vTexto) then
                raise Exception.Create(emTarTruncado);
              if vCab.Tipo = 'L' then
              begin
      ...
      if (vRel <> '') and ((Pos('..', vRel) > 0) or (vRel[1] = '/') or
                           (Pos(':', vRel) > 0)) then
      begin
        AErro := Format(wmTarEntradaSuspeita, [vNome]);
        vRel := '';
      end;
          ...
          begin
            ...
            try
              if not LerTudo(vGz, vTamanho, vSaida) then
                raise Exception.CreateFmt(emTarTruncadoEm, [vNome]);
            finally
              vSaida.Free;
            end;
            {$IFDEF UNIX}
