/// What the Delphi IDE reads from the registry when it opens: known packages,
/// library path and environment variables. Written directly with TRegistry,
/// with no IDE tool at all. Every write keeps the value before: it goes to the
/// receipt, and it is what uninstalling undoes. Nothing is deleted without
/// being noted.
...
uses
  ...
  RALInst.IDE, RALInst.Processo;

type
  /// One registry write, with the value before.
  ...
  /// Reads and writes the registry configuration of one Delphi IDE.
  ...
  private
    ...
    FLog: TLogLinha;
    FSimular: boolean;
    /// Notes one write, for the receipt
    ...
    /// Same folder, however written
    function MesmaPasta(const A, B: string): boolean;
  public
    constructor Create(AIDE: TIDEInstance);
    /// Does the IDE accept $(Name) in the library path? (Delphi 7 does not)
    function AceitaVariaveis: boolean;
    /// Appends to the end of a path list ('Search Path', 'Browsing Path'),
    /// without repeating what is already there written another way; returns
    /// how many entered, or -1 on error
    function AdicionarCaminhos(const APlataforma, AValor: string;
      ACaminhos: TStrings): integer;
    /// The noted writes, as the receipt stores them
    function AlteracoesJSON: TJSONArray;
    /// Folders of the list holding a unit named like one of ours that are not
    /// ours: another RAL copy, which will fight this one
    procedure CaminhosConflitantes(const APlataforma: string; ANossos,
      AUnidades, AResultado: TStrings);
    /// The IDE key exists in HKCU: without it the IDE was never opened, and
    /// writing before that disturbs its first start
    function ChaveExiste: boolean;
    /// 'Library\Win32' from XE2 on; 'Library' before (a single platform)
    function ChaveLibrary(const APlataforma: string): string;
    /// The IDE's own Environment Variables (not Windows')
    function DefinirVariavel(const ANome, AValor: string): boolean;
    /// Writes a value (noting the one before); False on error
    function Escrever(const ASubchave, AValor, AConteudo: string): boolean;
    /// $(BDS), $(BDSCOMMONDIR), $(Platform), the IDE's own variables and the
    /// system's; what it does not know stays as it is
    function Expandir(const ACaminho: string;
      const APlataforma: string = 'win32'): string;
    /// bds.exe (or delphi32.exe) of this installation running: what is written
    /// now the IDE rewrites when it closes, and the installation is lost
    function IDEEmExecucao: boolean;
    function LerValor(const ASubchave, AValor: string): string;
    /// Subkey names of a subkey ('Library' -> Win32, Win64...)
    procedure ListarSubchaves(const ASubchave: string; ALista: TStrings);
    /// Value names of a subkey
    procedure ListarValores(const ASubchave: string; ALista: TStrings);
    /// Known Packages; removes the same .bpl from Disabled Packages (the IDE
    /// disables a package that failed once and never tries again) and the
    /// registration of the same .bpl in another folder, which would make the
    /// IDE load two. AX64: the 64-bit IDE's list (Known Packages x64)
    function RegistrarPacote(const ABpl, ADescricao: string;
      AX64: boolean = False): boolean;
    /// Deletes a value (noting it); False on error
    ...
    /// Relative to HKCU; by default the IDE's. The tests point it to a copy
    property Chave: string read FChave write FChave;
    property IDE: TIDEInstance read FIDE;
    property Log: TLogLinha read FLog write FLog;
    /// Says what it would write, without writing
    property Simular: boolean read FSimular write FSimular;
    property TotalAlteracoes: integer read GetTotalAlteracoes;
  end;

/// Copies the string values of an HKCU key (and subkeys) into another; the
/// tests use it to work on a copy of the IDE registry
function CopiarChave(const AOrigem, ADestino: string): boolean;
/// Undoes, in reverse order, the registry writes noted in an installation
/// receipt (what existed goes back to its value; what was created is deleted).
/// The written .bpl files stay: that is the complete uninstall
function DesfazerRecibo(const AArquivo: string; ALog: TLogLinha): boolean;
/// Platform name as the Delphi registry writes it: win32 -> Win32
      ...
      try
        ...
        begin
          ...
          begin
            ...
            vReg.CloseKey;
            Logar(Format(cmRegistroRestaurado, [vChave + '\' + vValor]));
          end
          else
          begin
            Logar(Format(emRegistroAbrir, ['HKCU' + vChave]));
            Result := False;
          end;
        end
        ...
        begin
          ...
          begin
            ...
            Logar(Format(cmRegistroApagado, [vChave + '\' + vValor]));
          end;
          vReg.CloseKey;
        end;
      except
        on E: Exception do
        begin
          Logar(cmPrefixoErro + vChave + '\' + vValor + ': ' + E.Message);
          Result := False;
        end;
      end;
    end;
...
begin
  ...
  try
    ...
    try
      ...
      begin
        Logar(Format(emRegistroAbrir, ['HKCU' + FChave + '\' + ASubchave]));
        Exit;
      end;
      ...
      on E: Exception do
        Logar(Format(emRegistroGravar, [ASubchave + '\' + AValor, E.Message]));
    end;
  finally
    vReg.Free;
  end;
...
function TRegistroDelphi.Expandir(const ACaminho: string;
...
begin
  ...
  begin
    if vAntes = '' then
      Logar(Format(cmRegistroVariavel, [ANome, AValor]))
    else
      Logar(Format(cmRegistroVariavelEra, [ANome, AValor, vAntes]));
  end;
end;
...
begin
  ...
  try
    ...
    begin
      ...
      vExpandido := LowerCase(ExcludeTrailingPathDelimiter(
                      Expandir(ACaminhos[vInt], APlataforma)));
      ...
      Logar(Format('  %s %s += %s',
                   [PlataformaRegistro(APlataforma), AValor, ACaminhos[vInt]]));
      Inc(Result);
    end;
...
function TRegistroDelphi.RegistrarPacote(const ABpl, ADescricao: string;
  AX64: boolean): boolean;
var
  vValores: TStringList;
  vInt, vAntes: integer;
  vNomeBpl, vKnown, vDisabled: string;
begin
  vNomeBpl := ExtractFileName(ABpl);
  // a IDE de 64 bits (bin64\bds.exe, do Delphi 12 em diante) tem as listas dela
  vKnown := 'Known Packages';
  vDisabled := 'Disabled Packages';
  if AX64 then
  begin
    vKnown := vKnown + ' x64';
    vDisabled := vDisabled + ' x64';
  end;
  vValores := TStringList.Create;
  try
    // o mesmo .bpl registrado de outra pasta: a IDE carregaria os dois e
    // recusaria o segundo por unidade duplicada
    ListarValores(vKnown, vValores);
    for vInt := 0 to Pred(vValores.Count) do
      if SameText(ExtractFileName(Expandir(vValores[vInt])), vNomeBpl) and
         not MesmaPasta(Expandir(vValores[vInt]), ABpl) then
      begin
        Logar(Format(cmRegistroOutraPasta, [vKnown, vValores[vInt]]));
        Remover(vKnown, vValores[vInt]);
      end;

    // pacote que falhou ao carregar uma vez fica desabilitado para sempre,
    // mesmo depois de corrigido
    ListarValores(vDisabled, vValores);
    for vInt := 0 to Pred(vValores.Count) do
      if SameText(ExtractFileName(Expandir(vValores[vInt])), vNomeBpl) then
      begin
        Logar('  ' + vDisabled + ' -= ' + vValores[vInt]);
        Remover(vDisabled, vValores[vInt]);
      end;
  finally
    vValores.Free;
  end;
  ...
  Result := Escrever(vKnown, ABpl, ADescricao);
  if Result and (Length(FAlteracoes) > vAntes) then
    Logar('  ' + vKnown + ' += ' + ABpl)
  else if Result then
    Logar(Format(cmRegistroJaRegistrado, [vKnown, vNomeBpl]));
end;

procedure TRegistroDelphi.ListarSubchaves(const ASubchave: string; ALista: TStrings);
var
  vReg: TRegistry;
begin
  ALista.Clear;
