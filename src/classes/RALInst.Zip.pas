/// The zip of a GitHub version in two roles: catalog source without extracting
/// anything, for the choice screens, and extraction source into the chosen
/// folder, at run time. Every GitHub zip has a top folder ('PascalRAL-1.1/'),
/// which is dropped in both cases.
unit RALInst.Zip;
...
uses
  ...
  RALInst.Catalogo, RALInst.Processo;

type
  /// Catalog source read straight from a zip file.
  TOrigemZip = class(TOrigemArquivos)
  private
    FArquivo: string;
    FCommit: string;
    FDescricao: string;
    /// relative path (no top folder, with '/') = name inside the zip
    FEntradas: TStringList;
    FLido: TMemoryStream;
    /// Unzipper callback: the entry is read into FLido
    procedure CriarStream(Sender: TObject; var AStream: TStream;
      AItem: TFullZipFileEntry);
    /// Unzipper callback: keeps FLido alive
    procedure FecharStream(Sender: TObject; var AStream: TStream;
      AItem: TFullZipFileEntry);
  public
    /// Raises an exception when the file is not a readable zip
    constructor Create(const AArquivo, ADescricao: string);
    destructor Destroy; override;
    /// A GitHub zip has no submodules: they are downloaded apart
    ...
    /// Commit of the zipball (GitHub writes it in the zip comment); '' if none
    property Commit: string read FCommit;
  end;

/// Comment of a zip file ('' when there is none or it is not a zip)
function ComentarioZip(const AArquivo: string): string;
/// Extracts dropping the first ARetirarNiveis folders of every entry (1 = the
/// top folder of a GitHub zip); refuses entries that leave the destination
/// ('..', absolute paths)
...
begin
  ...
  begin
    Erro := Format(wmZipEntradaSuspeita, [AItem.ArchiveFileName]);
    Exit;
  end;
...
begin
  ...
  try
    ...
    try
      ...
      on E: Exception do
        AErro := Format(emZipExtrair, [ExtractFileName(AArquivo), E.Message]);
    end;
  finally
    vExt.Free;
    vZip.Free;
...
begin
  ...
  for vInt := 0 to Pred(FEntradas.Count) do
    if (vPasta = '') or
       SameText(Copy(FEntradas.Names[vInt], 1, Length(vPasta)), vPasta) then
      ALista.Add(FEntradas.Names[vInt]);
end;

function TOrigemZip.Existe(const ACaminho: string): boolean;
begin
  Result := FEntradas.IndexOfName(StringReplace(ACaminho, '\', '/',
                                                [rfReplaceAll])) >= 0;
end;
...
begin
  ...
  if vIdx < 0 then
    raise EFileNotFoundException.CreateFmt(emZipNaoExiste, [ACaminho, FDescricao]);
  ...
  try
    ...
    if FLido = nil then
      raise EFileNotFoundException.CreateFmt(emZipNaoLido, [ACaminho, FDescricao]);
    FLido.Position := 0;
