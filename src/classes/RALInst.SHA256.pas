/// SHA-256 (FIPS 180-4) to check the binary downloaded by the self-update
/// against the release's SHA256SUMS. FPC's fpsha256 only exists from 3.2.3 on,
/// and the official Lazarus ships with 3.2.2.
unit RALInst.SHA256;
...
/// Hash of a file, in lowercase hexadecimal as sha256sum writes it
function SHA256Arquivo(const AArquivo: string): string;
/// Hash of a string, in lowercase hexadecimal as sha256sum writes it
...
begin
  ...
  Inc(AEstado.H[0], A); Inc(AEstado.H[1], B);
  Inc(AEstado.H[2], C); Inc(AEstado.H[3], D);
  Inc(AEstado.H[4], E); Inc(AEstado.H[5], F);
  Inc(AEstado.H[6], G); Inc(AEstado.H[7], H);
end;
