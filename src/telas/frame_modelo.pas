...
type
  ...
  Tfframe_modelo_class = class of Tfframe_modelo;
...
procedure Tfframe_modelo.lbNextClick(Sender : TObject);
begin
  if lbNext.Enabled and Assigned(FOnNext) then
    FOnNext(Self);
end;

procedure Tfframe_modelo.FrameResize(Sender : TObject);
begin
  recalcAligns;
end;

procedure Tfframe_modelo.lbBackClick(Sender : TObject);
begin
  if lbBack.Enabled and Assigned(FOnBack) then
    FOnBack(Self);
end;

procedure Tfframe_modelo.SetTheme(AValue : TThemes);
var
  vInt : Integer;
begin
  ...
  begin
    if (Components[vInt] is TLabel) and (TLabel(Components[vInt]).Tag = 0) then
      ...
      TImage(Components[vInt]).Picture.LoadFromResourceName(HInstance, Themes[AValue].Button);
  end;

  FTheme := AValue;
end;

constructor Tfframe_modelo.Create(TheOwner : TComponent);
begin
  inherited Create(TheOwner);
  recalcAligns;
end;

procedure Tfframe_modelo.recalcAligns;
begin
  lbSubTitle.Left := (Self.Width div 2) - (lbSubTitle.Width div 2);
end;

procedure Tfframe_modelo.validaControls;
begin
