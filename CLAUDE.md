...
Reading the version back is only reliable **on the server**. A Windows client has to guess from the status line, which an HTTP/2 response does not have; the http.sys server reads it from the driver — and not from `HTTP_REQUEST.Version`, which stays 1.1 even for h2, but from `HTTP_REQUEST_FLAG_HTTP2`.

`TRALClient.ShareConnection` is the other half of the gain: one connection for every client aimed at the same place with the same settings, instead of one per dataset. It is a hint, not a contract — engines that cannot share ignore it. netHTTP honours it with a transport pool of its own (whose key carries the certificate policy, so only clients that judge certificates alike ever share), okhttp by handing the question to OkHttp's client cache.

### `TRALSynopseServer.Mode`

`smThreads` is what the engine always did and stays the default (a thread per kept-alive connection, ceiling at `MaxKeepAlive`). `smAsync` runs mORMot2's event loop instead — IOCP/epoll — so idle connections cost a socket rather than a thread. `smHttpSys` hands the sockets to the Windows kernel and is the only mode that serves HTTP/2; its certificate comes from the machine store through `netsh http add sslcert`, **not** from `SSL.CertificateFile`, and the URL has to be reserved once with `netsh http add urlacl` or `AddUrl` fails with code 5.
