...
One trap worth knowing on Windows: asking for h2 also caps the transport at one connection (`MAX_CONNS_PER_SERVER`), which is what turns h2 into multiplexing - but that cap is applied **only after a response confirms h2**, never because it was asked for. Under HTTP/1.1 a single connection queues concurrent requests instead of running them, so capping on the request made "ask for h2" slower than asking for nothing whenever the other side did not have it. Which engines can is `SupportsSharedConnection`, a class function like the other two - the ones that cannot are not slower for ignoring it, they would be slower for honouring it, since one object there means one socket and sharing would serialise concurrent calls.
