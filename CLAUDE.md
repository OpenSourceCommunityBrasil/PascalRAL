...
`TRALClient.SSL.Pin` (SHA-256 of the expected certificate), `SSL.Required` (refuse plain http) and `OnValidateServerCert` are the whole surface, and they live on `TRALClient` — never on an engine, which is what keeps them identical whatever `EngineType` is set to. Each engine only translates its own callback into `TRALCertInfo` — the same record on every compiler and platform — and asks `TRALClientHTTP.AcceptServerCert`, where the single rule lives: **the event decides, else the pin, else what the engine itself concluded**. Same shape as `SetTransportError` for retries.
