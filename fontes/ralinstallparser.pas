...
uses
  Classes, SysUtils, fpJSON, fpjsonrtti, githubutils;
...
begin
  ...
  {$IFDEF INSTALL_TEST}
    ...
    vPath := vPath + 'tests/install.json';
    ...
    begin
      ...
      try
        ...
        SetLength(vJSON, vStream.Size);
        vStream.Read(vJSON[1], vStream.Size);
      finally
        FreeAndNil(vStream);
      end;
    end;
  ...
  if vJSON <> '' then
  begin
    vJsonDS := TJSONDeStreamer.Create(nil);
    try
      vJsonDS.JSONToObject(vJSON, Self);
    finally
      FreeAndNil(vJsonDS);
    end;
  end;
